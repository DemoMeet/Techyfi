...
import 'package:groceyfi02/pages/return/widgets/trashbin_return_list_item.dart';
...
class TrashItemsList extends StatefulWidget {
  ...
  TrashItemsList(
      { required this.nn});
  @override
  State<TrashItemsList> createState() => _TrashItemsListState();
}

class _TrashItemsListState extends State<TrashItemsList> {
  ...
  List<bool> _click = [];
  List<Trashbin> allInvoiceLists = [];
  List<Trashbin> foundInvoiceLists = [];
  ...
  void _fetchDocuments() async {
    ...
    await FirebaseFirestore.instance
      ...
      querySnapshot.docs.forEach((element) {
        allInvoiceLists.add(Trashbin(returndate: element["Return Date"].toDate(),products: element["Products"],receivedamount: element["Received Amount"],returnid: element["Return ID"],supplierid: element["Supplier ID"],suppliername: element["Supplier Name"],totalamount: element["Total Amount"],totaldeduction: element["Total Deduction"],trash: element["Trash"],
            sl: s));
        s++;
      });
    });
  ...
  void _runFilter(String enteredKeyword) {
    List<Trashbin> results = [];
    if (enteredKeyword.isEmpty) {
      results = allInvoiceLists;
    } else {
      results = allInvoiceLists
          .where((user) => user.suppliername
          .toLowerCase()
  ...
  void _findData() {
    if (startDate.compareTo(endDate) > 0) {
      ...
      for (int i = 0; i < allInvoiceLists.length; i++) {
        if ((allInvoiceLists[i].returndate.compareTo(startDate) >= 0 &&
            allInvoiceLists[i].returndate.compareTo(endDate) <= 0) ||
            (allInvoiceLists[i].returndate.difference(startDate).inDays == 1 ||
                allInvoiceLists[i].returndate.difference(endDate).inDays ==
                    1)) {
          results.add(allInvoiceLists[i]);
        } else if (startDate.difference(endDate).inDays == 0) {
          if (allInvoiceLists[i].returndate.compareTo(startDate) == 1) {
            results.add(allInvoiceLists[i]);
          }
        }
      }
  ...
  Widget build(BuildContext context) {
    ...
    Future<List<Trashbin>> getCust() async {
      List<Trashbin> InvoiceLists = [];
                                                ...
                                                setState(() {
                                                  if (!_totaltop) {
                                                    ...
                                                    allInvoiceLists.sort((a, b) =>
                                                        a.receivedamount.compareTo(b.receivedamount));
                                                    makechunks();
                                                  } else {
                                                    _datetop = false;
                                                    _datebot = false;
