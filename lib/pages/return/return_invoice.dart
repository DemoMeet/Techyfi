...
class _ReturnInvoiceState extends State<ReturnInvoice> {
  ...
  Future<void> _save() async {
    ...
    if (
      ...
      Get.snackbar("Invoice Return Failed.",
          ...
          borderRadius: 0);
    } else {if (_selectedAccountid.bal > nettotal){
      if (_listitem
            .where((element) =>
      ...
      if (_sts == 2) {
        ...
        for (int i = 0; i < _listupdated.length; i++) {
          ...
          FirebaseFirestore.instance
              .collection('Wastage')
              .doc(ss.productId).get().then((value) {
                if(value.exists){
                  FirebaseFirestore.instance
                      .collection('Wastage')
                      .doc(ss.productId)
        ...
        FirebaseFirestore.instance.collection('Transaction').add({
          ...
          'Remarks': "Invoice Return (Wastage)",
        ...
        FirebaseFirestore.instance.collection('Return').doc(rtnID).set({
          ...
          'Invoice ID': widget.cst.invoiceID,
          'Purchase ID': '',
          'Customer Name': widget.cst.customerName,
          'User': AuthService.to.user?.name,
          'Supplier Name': '',
          ...
          'Purchase': false,
          'Wastage': true,
          'Return Type': "Wastage Sale Return",
          ...
          Navigator.of(context)
              .pushReplacementNamed(invoicereturnlistPageRoute);
        ...
        for (int i = 0; i < _listupdated.length; i++) {
          ...
          FirebaseFirestore.instance
              .collection('Stock')
              .doc(ss.productId).get()
              .then((val){
            FirebaseFirestore.instance
                .collection('Stock')
                .doc(ss.productId)
                .update({
              'Quantity': val['Quantity'] + double.parse(ss.returnqtycontrollers.text),
            });
          });
        }
