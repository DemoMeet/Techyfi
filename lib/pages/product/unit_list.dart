...
class _UnitListState extends State<UnitList> {
 ...
 late ScrollController listScrollController;
  ...
  List<bool> _click = [];
  List<General> allunit = [];
  List<General> foundunit = [];
  TextEditingController search_unit = TextEditingController();
  ...
  void _fetchDocuments() async {
    ...
    await FirebaseFirestore.instance
      ...
      querySnapshot.docs.forEach((element) {
        allunit.add(General(name: element["Name"],
  ...
  void makechunks() {
    chunks = [];
    for (var i = 0; i < foundunit.length; i += _items) {
      chunks.add(foundunit.sublist(
          i, i + _items > foundunit.length ? foundunit.length : i + _items));
    }
    setState(() {
      _totalpagenum = (foundunit.length / _items).floor() + 1;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    async  {
      var result = await Get.toNamed(editunitPageRoute,arguments: {
                                        ...
                                        child: InkWell(
                                          onTap: () {
                                            setState(() {
                                              _nametop = false;
                                              _namebot = false;
                                              allunit.sort((a, b) => a.sl.compareTo(b.sl));
                                              makechunks();
                                            });
                                          },
                                          child: Text(
                                            ...
                                            onTap: () {
                                              setState(() {
                                                if(!_nametop){
                                                  _nametop = true;
                                                  _namebot = false;
                                                  allunit.sort((a, b) => a.name.compareTo(b.name));
                                                  makechunks();
                                                }else{
                                                  _nametop = false;
                                                  _namebot = true;
                                                  allunit.sort((b, a) => a.name.compareTo(b.name));
                                                  makechunks();
                                                }
                                              });
                                            },
