...
class _EditCategoryState extends State<EditCategory> {
  final _conCategoryname = TextEditingController();
  bool isfirst = true;

  int _sts = 1;
  void _addinit(General gs) {
    print(_sts);
    _conCategoryname.text = gs.name;
    if (gs.status == "Active") {
      _sts = 1;
    } else {
      _sts = 2;
  ...
  _editCategory(General gs) async {
    String cname = _conCategoryname.text;
    bool cstats = true;
    if (_sts == 2) {
      cstats = false;
    }
    ...
    if (cname.isEmpty) {
      ...
      FirebaseFirestore.instance.collection('Category').doc(gs.id).update({
        'Name': cname,
        'status': cstats,
      }).then((value) {
        Get.back();
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: Column(
          ...
          children: [
            CustomText(text: "Edit Category", size: 32, weight: FontWeight.bold),
            Container(
