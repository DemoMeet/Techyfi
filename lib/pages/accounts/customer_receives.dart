...
class CustomerReceives extends StatefulWidget {
  
  Navbools nn;
  CustomerReceives({required this.nn});
  @override
  State<CustomerReceives> createState() => _CustomerReceivesState();
}

class _CustomerReceivesState extends State<CustomerReceives> {
  final ScreenSizeController controller = Get.put(ScreenSizeController());
  var _selectedcustomer;
  var _selectedcustomerid;
  var customer = [], scustomer = [];
  final _convoucherid = TextEditingController(),
      _concustomerAmount = TextEditingController();
  ...
  _addunit() async {
    ...
    double amount = double.parse(_concustomerAmount.text.toString());
    if (_selectedpayment.toString() == "null" ||
        _selectedcustomer.toString() == "null" ||
        _selectedAccountid.toString() == "null" ||
        _conremarks.text.isEmpty ||
        _concustomerAmount.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text("All Fields are Required to Process customer!!"),
      ));
    } else {
      FirebaseFirestore.instance.collection('Transaction').add({
        'Name': _selectedcustomerid.name,
        '2nd ID': _selectedcustomerid.id,
        'Remarks': "Customer Receives",
      ...
      FirebaseFirestore.instance.collection('PaymentVoucher').doc(ss).set({
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          ...
          content: Text("Customer Receives Added Successfully!"),
        ));
    ...
    await FirebaseFirestore.instance
      ...
      querySnapshot.docs.forEach((element) {
        customer.add(Customer(
            ...
            email: element["Email"],
            id: element.id,
            name: element["Name"],user: element["User"],
            phone1: element["Phone"],
            phone2: element["Phone2"],
            zip: element['Zip Code'],
            sl: 0));
        scustomer.add(element["Name"]);
        setState(() {});
      });
    });
        ...
        setState(() {
          if (element["Bank"]) {
            _bankaccounts.add(Accounts(
                ...
                bal: element["Balance"],
                sts: element["Status"],
                bankname: element["Bank Name"],user: element['User'],
                bank: element["Bank"],
                          ...
                          Expanded(
                            ...
                            child: Container(
                              ...
                              child: DropdownButton(
                                ...
                                onChanged: (newValue) {
                                  setState(() {
                                    int indx = scustomer.indexOf(newValue);
                                    _selectedcustomerid = customer[indx];
                                    _selectedcustomer = newValue.toString();
                                    _dueAmount = _selectedcustomerid.balance;
                                  });
                                },
                                items: customer.map((location) {
