...
class PdfTransaction {
  static Future<File> generate(List<Transactionss> invoice) async {
    final pdf = Document();
    ...
    return PdfApi.saveDocument(
        "Transaction_List${DateFormat('dd-MMM-yyyy-jms').format(DateTime.now())}.pdf",
        pdf);
  }
  ...
  static Widget buildInvoice(List<Transactionss> invoice, final ttf, final ttfbold) {
    final headers = [
      '#',
      'Bank /Cash Name',
      'Bearer',
    ...
    final data = invoice.map((item) {
      return [
        (item.sl + 1).toString(),
        item.account["Bank"]?"${item.account["Bank Name"]} (${item.account["Account Number"]})":item.account["Cash Name"],
        item.name,
        item.type,
